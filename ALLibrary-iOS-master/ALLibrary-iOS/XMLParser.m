...
- (void)parser:(NSXMLParser *)parser parseErrorOccurred:(NSError *)parseError {
    dispatch_async(dispatch_get_main_queue(), ^{ self.failure(parseError); });
}

- (void)             parser:(NSXMLParser *)parser
    validationErrorOccurred:(NSError *)validError {
    dispatch_async(dispatch_get_main_queue(), ^{ self.failure(validError); });
}
...
- (void)     parser:(NSXMLParser *)parser
    ...
    if ([self.currentNode objectForKey:elementName]) {
        NSMutableArray *elementsArray = [NSMutableArray
                                         arrayWithObjects:[self.currentNode objectForKey:elementName],
                                         [XMLParser cleanedAttributes:attributeDict], nil];
    ...
    else if ([self.currentNode
              objectForKey:[elementName
                            stringByAppendingString:@"sArray"]]) {
                  NSMutableArray *currentNode = [self.currentNode
                                                 objectForKey:[elementName stringByAppendingString:@"sArray"]];
                  [currentNode addObject:[XMLParser cleanedAttributes:attributeDict]];
                  currentNodeKey = [elementName stringByAppendingString:@"sArray"];
              }
    else {
        currentNodeKey = elementName;
        [self.currentNode setObject:[XMLParser cleanedAttributes:attributeDict]
                             forKey:currentNodeKey];
    }
...
- (void)   parser:(NSXMLParser *)parser
    ...
    if ([self.foundCharacters length] > 0 &&
        !NSStringEqualsXMLNullString(self.foundCharacters)) {
        [self.currentNode
         setObject:[self.foundCharacters
                    stringByTrimmingCharactersInSet:
                    [NSCharacterSet whitespaceAndNewlineCharacterSet]]
         forKey:@"content"];
        [self setFoundCharacters:nil];
    }
    else if ([self.foundCDATA length] > 0 &&
             !NSStringEqualsXMLNullString(self.foundCharacters)) {
        NSString *foundCDATA = [[NSString alloc] initWithData:self.foundCDATA
                                                     encoding:NSUTF8StringEncoding];
        [self.currentNode
         setObject:[foundCDATA
                    stringByTrimmingCharactersInSet:
                    [NSCharacterSet whitespaceAndNewlineCharacterSet]]
         forKey:@"content"];
