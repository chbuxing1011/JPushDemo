...
TableViewNumberOfRowsInSectionConfigurate numberOfRowsConfigure;
...
- (id)                    initWithItems:(NSArray *)aItems
    ...
    numberOfRowsInSectionConfigureBlock:
(TableViewNumberOfRowsInSectionConfigurate)
aNunberOfRowsInSectionConfigureBlock
                     cellConfigureBlock:
(TableViewCellConfigurate)aCellConfigureBlock;
{
    self = [super init];
    if (self) {
        self.items = [aItems copy];
...
- (void)          tableView:(UITableView *)tableView
    ...
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(selectRowAtIndexPath:)]) {
