...
(UMSocialResponseEntity *)response {
    NSLog(@"didFinishGetUMSocialDataInViewController with response is %@",
          response);
    //根据`responseCode`得到发送结果,如果分享成功
    if (response.responseCode == UMSResponseCodeSuccess) {
        //得到分享到的微博平台名
        NSLog(@"share to sns name is %@",
              [[response.data allKeys] objectAtIndex:0]);
    }
}
...
/*
 注意分享到新浪微博我们使用新浪微博SSO授权，你需要在xcode工程设置url
 scheme，并重写AppDelegate中的`- (BOOL)application openURL
 sourceApplication`方法，详细见文档。否则不能跳转回来原来的app。
 */
- (IBAction)showShareList1:(id)sender {
    //    NSString *shareText =
    //    @"友"
    //    @"盟社会化组件可以让移动应用快速具备社会化分享、登录、评论、喜欢等功能，"
...
clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    NSString *shareText =
    @"友" @"盟"
    @"社会化组件可以让移动应用快速具备社会化分享、登录、"
    @"评"
    @"论、喜欢等功能，并提供实时、全面的社会化数据统计分"
...
- (void)viewDidLoad {
    _shareButton1.center = CGPointMake(self.view.frame.size.width / 2,
                                       self.view.frame.size.height / 6);
    _shareButton2.center = CGPointMake(self.view.frame.size.width / 2,
                                       self.view.frame.size.height / 6 * 2);
    _shareButton3.center = CGPointMake(self.view.frame.size.width / 2,
                                       self.view.frame.size.height / 6 * 3);
    _shareButton4.center = CGPointMake(self.view.frame.size.width / 2,
                                       self.view.frame.size.height / 6 * 4);
...
(UIInterfaceOrientation)toInterfaceOrientation
     ...
     */
    UIView *iconActionSheet =
    [self.tabBarController.view viewWithTag:kTagSocialIconActionSheet];
    
    [iconActionSheet setNeedsDisplay];
    UIView *shakeView =
    [self.tabBarController.view viewWithTag:kTagSocialShakeView];
